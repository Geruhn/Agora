...
module.exports = {
  sendMailForNewMember: function (member, callback) {
    var message = new Message({
      subject: 'Die Softwerkskammer hat ein neues Mitglied',
      markdown: 'Es heißt: ' + member.firstname + ' "' + member.nickname + '" ' + member.lastname + '\n' +
      'Du bekommst die Mail, weil du in der Admin-Liste stehst, die darüber informiert werden soll.\n' +
      'Wenn du diese Mail nicht mehr bekommen möchtest, melde dich bei einem (anderen) Administrator oder leg selber Hand an.',
      htmlAddOn: ''
    }, new Member({
      firstname: 'I,',
      nickname: '',
    ...
    this.sendMailToAdressList(conf.get('adminList'), message, callback);
  },
  ...
  sendMailToAdressList: function (adressList, message, callback) {
    if (!adressList || adressList.length === 0) {
      return callback();
    }
    message.setBcc(adressList);
    sendMail(message, 'Nachricht', callback);
  }
};
